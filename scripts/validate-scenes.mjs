...
function validateStats(dataDir) {
...
function validateItems(dataDir, registries) {
...
function validateScenes(dataDir, registries) {
  const sceneDir = path.join(dataDir, 'scenes')
...
function validateEndings(dataDir, registries) {
  const file = path.join(dataDir, 'endings.json')
  const data = readJson(file)
...
function main() {
  if (!existsSync(DATA_DIR)) {
    ...
    registries.statKeys = validateStats(DATA_DIR)
    registries.itemIds = validateItems(DATA_DIR, registries)
    validateScenes(DATA_DIR, registries)
    validateEndings(DATA_DIR, registries)
  }
